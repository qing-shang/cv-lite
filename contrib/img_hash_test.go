...
func compute(path string, hash ImgHashBase) (*cv_lite.Mat, error) {
	img := cv_lite.IMRead(path, cv_lite.IMReadColor)
	if img.Empty() {
...
func BenchmarkCompute(b *testing.B) {
	img := cv_lite.IMRead(testImage, cv_lite.IMReadColor)
	if img.Empty() {
