...
func main() {
	...
	mat := cv_lite.IMRead(filename, cv_lite.IMReadColor)

	matCanny := cv_lite.NewMat()
	matLines := cv_lite.NewMat()

	window := cv_lite.NewWindow("detected lines")

	cv_lite.Canny(mat, &matCanny, 50, 200)
	cv_lite.HoughLinesP(matCanny, &matLines, 1, math.Pi/180, 80)
	...
	for i := 0; i < matLines.Rows(); i++ {
		...
		cv_lite.Line(&mat, pt1, pt2, color.RGBA{0, 255, 0, 50}, 10)
	}
