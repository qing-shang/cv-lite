...
func main() {
	...
	filename := os.Args[1]
	img := cv_lite.IMRead(filename, cv_lite.IMReadColor)
	...
	corners := cv_lite.NewMat()
	defer corners.Close()
	sz := image.Point{X: 4, Y: 6}
	found := cv_lite.FindChessboardCorners(img, sz, &corners, 0)
	if found == false {
	...
	clone := img.Clone()
	defer clone.Close()
	cv_lite.DrawChessboardCorners(&clone, sz, corners, found)
	if clone.Empty() {
		fmt.Printf("Error writing to chessboard image")
		return
	}

	window := cv_lite.NewWindow("Chessboards")
	defer window.Close()
	for {
		window.IMShow(clone)
