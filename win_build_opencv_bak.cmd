...
if not exist "c:\opencv\opencv-4.7.0.zip" (powershell -command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri https://github.com/opencv/opencv/archive/4.7.0.zip -OutFile c:\opencv\opencv-4.7.0.zip")
echo Extracting...
if not exist "c:\opencv\opencv-4.7.0"  powershell -command "$ProgressPreference = 'SilentlyContinue'; Expand-Archive -Path c:\opencv\opencv-4.7.0.zip -DestinationPath c:\opencv"
REM del c:\opencv\opencv-4.7.0.zip /q
echo.

echo Downloading: opencv_contrib-4.7.0.zip [58MB]
if not exist "c:\opencv\opencv_contrib-4.7.0.zip" (powershell -command "[Net.ServicePointManager]::SecurityProtocol = [Net.SecurityProtocolType]::Tls12; $ProgressPreference = 'SilentlyContinue'; Invoke-WebRequest -Uri https://github.com/opencv/opencv_contrib/archive/4.7.0.zip -OutFile c:\opencv\opencv_contrib-4.7.0.zip")
echo Extracting...
if not exist "c:\opencv\opencv_contrib-4.7.0" powershell -command "$ProgressPreference = 'SilentlyContinue'; Expand-Archive -Path c:\opencv\opencv_contrib-4.7.0.zip -DestinationPath c:\opencv"
REM del c:\opencv\opencv_contrib-4.7.0.zip /q
echo.
...
set PATH=%PATH%;D:\UseApp\CMake\bin;C:\mingw64\bin
if [%1]==[static] (
